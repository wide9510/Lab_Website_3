...
app.get('/team_stats', function(req,res){
		var query1 = 'select visitor_name,home_score,visitor_score, game_date, players from football_games';
		var query2 = 'select Count(home_score) as home_count from football_games where home_score > visitor_score;';
		var query3 = 'select Count(home_score) as home_count2 from football_games where home_score < visitor_score;';
		db.task('get-everything', task => {
			return task.batch([
				task.any(query1),
				task.any(query2),
...
app.get('/player_info/select_player', function(req,res){
	...
	var query3 = 'select count(football_players.id) as games_played from football_players inner join football_games on football_players.id = any(football_games.players) where football_players.id = '+player_id+';';
	db.task('get-everything', task => {
		return task.batch([
			task.any(query1),
			task.any(query2),
